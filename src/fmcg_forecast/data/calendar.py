...
def create_feature_calendar(
    ...
    df["is_rainy_season"] = (
        df["date"].dt.month.isin([10, 11, 12, 1, 2, 3, 4]).astype(int)
    )
    df["is_payday_period"] = (
    ...
    for year in range(start.year, end.year + 1):
        ...
        df.loc[
            (df["date"] >= ramadan_start) & (df["date"] < lebaran), "is_ramadan_month"
        ] = 1
        df.loc[
    ...
    logger.info(
        "Calendar generated: %d days, %d features", len(df), len(df.columns) - 1
    )
    return df
